...
var force_accel = 10
var unit_img = preload("res://Images/Arbeiter.jpg")
...
func get_closest_availible_enemy_unit(group_name):
...
func search_for_enemies(group_name):
	if current_state == states.IDLE or current_state == states.ATTACKING:
		get_closest_availible_enemy_unit(group_name)
...
func follow_path(): # für patroullie
	if get_parent() is Path3D:
		var curve = get_parent().get_curve()
		var has_point = current_path_index != -1
		if !has_point:
			var closest_distance
			for index in curve.get_point_count():
				var distance_between = global_transform.origin.distance_to(curve.get_point_position(index))
				if closest_distance == null || distance_between < closest_distance:
					closest_distance = distance_between
					current_path_index = index
				
		if has_point:
			current_path_index = ( current_path_index + 1 ) % curve.get_point_count()
		
		move_to(curve.get_point_position(current_path_index))
